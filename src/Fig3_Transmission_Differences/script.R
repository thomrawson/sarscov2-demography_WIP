...
versions <- c("Factual", "Counterfactual_principal", "Counterfactual_low_migration", 
...
for(i in 1:nrow(lookup)){
  ...
  } else if(lookup$version[i] == "Counterfactual_principal"){
    if(lookup$output_type[i] == "sympt_cases_under15_inc"){
...
s1 <- s1 + ggtitle("Pillar 2 Symptomatic Cases by Age (2019 Baseline)")

######################################
# Supplementary 2 - Symptomatic Cases over time, but split up by age, 2047 Counterfactual
s2 <- plot_sympt_cases_age_stack(simulation_data, "england", "Counterfactual_principal")
