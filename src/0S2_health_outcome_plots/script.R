...
plot_columns <- data.frame(index_names = c("deaths_hosp_inc",
                           ...
                           data_names = c("ons_death_hospital",
                                          "ons_death_hosp_0_49",
...
for(i in 1:nrow(plot_columns)){
  for(r in regions){
    ...
    plot_data <- england_data %>%
      select(date, region, plot_columns$data_names[i]) %>%
      filter(region == r)
    colnames(plot_data) <- c("time", "region", "value")
